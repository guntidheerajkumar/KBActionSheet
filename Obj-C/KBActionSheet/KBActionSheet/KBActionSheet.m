...
#import "KBActionSheet.h"

typedef NS_ENUM(NSInteger, KBActionSheetArrowDirection) {
    KBActionSheetArrowDirectionNone,
    KBActionSheetArrowDirectionDown,
    KBActionSheetArrowDirectionLeft,
    KBActionSheetArrowDirectionUp,
    KBActionSheetArrowDirectionRight
};

const CGFloat KBActionSheetButtonHeight  = 56.0f;
const CGFloat KBActionSheetDefaultWidth  = 300.0f;
const CGFloat KBActionSheetBorderRadius  = 15.0f;
const CGFloat KBActionSheetTitlePadding  = 15.0f;
const CGFloat KBActionSheetCompactMargin = 20.0f;
const CGFloat KBActionSheetArrowBase     = 36.0f;
const CGFloat KBActionSheetArrowHeight   = 13.0f;
const CGFloat KBActionSheetScreenPadding = 20.0f;
...
@property (nonatomic, assign) KBActionSheetArrowDirection arrowDirection;

/* View setup */
- (void)setUp;
- (void)setColorsForStyle:(KBActionSheetStyle)style;
- (void)show;
...
- (KBActionSheetArrowDirection)bestArrowDirectionForPresentationRect:(CGRect)presentationRect;
- (CGRect)frameOfContainerViewWithArrowDirection:(KBActionSheetArrowDirection)direction presentationRect:(CGRect)presentationRect;
- (CGRect)frameOfArrowViewWithDirection:(KBActionSheetArrowDirection)direction presentationRect:(CGRect)presentationRect;
- (UIColor *)bestColorForArrowWithDirection:(KBActionSheetArrowDirection)direction;
...
- (UIImage *)arrowImageForDirection:(KBActionSheetArrowDirection)direction color:(UIColor *)color;
...
- (void)setUp
{
    ...
    _cancelButtonTitle = NSLocalizedStringFromTable(@"Cancel", @"KBActionSheetLocalizable", @"Cancel Button");
...
- (void)setColorsForStyle:(KBActionSheetStyle)style
{
    if (style == KBActionSheetStyleDark) {
...
- (void)layoutSubviews
{
    ...
    if (self.compactLayout) {
        ...
        frame.origin.y = (self.frame.size.height - KBActionSheetCompactMargin) - frame.size.height;
        self.cancelButton.frame = frame;
        
        frame = self.containerView.frame;
        frame.size.width = self.width;
        frame.origin.x = (self.frame.size.width - frame.size.width) * 0.5f;
        frame.origin.y = (self.cancelButton.frame.origin.y - KBActionSheetCompactMargin) - frame.size.height;
        ...
        KBActionSheetArrowDirection direction = [self bestArrowDirectionForPresentationRect:presentationRect];
...
{
    //Set canvas size
    CGRect rect = CGRectMake(0, 0, (KBActionSheetBorderRadius * 2) + 1, (KBActionSheetBorderRadius * 2) + 1);
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:rect
                                               byRoundingCorners:corners
                                                     cornerRadii:(CGSize){KBActionSheetBorderRadius, KBActionSheetBorderRadius}];
    ...
    UIEdgeInsets edgeInsets = (UIEdgeInsets){KBActionSheetBorderRadius, KBActionSheetBorderRadius, KBActionSheetBorderRadius, KBActionSheetBorderRadius};
...
- (UIImage *)arrowImageForDirection:(KBActionSheetArrowDirection)direction color:(UIColor *)color
{
    ...
    if (direction == KBActionSheetArrowDirectionDown || direction == KBActionSheetArrowDirectionUp)
        size = (CGSize){KBActionSheetArrowBase, KBActionSheetArrowHeight};
    else
        size = (CGSize){KBActionSheetArrowHeight, KBActionSheetArrowBase};
    ...
    UIGraphicsBeginImageContextWithOptions(size, NO, 0.0f);
    {
        ...
        switch (direction) {
            case KBActionSheetArrowDirectionDown:
                ...
                break;
            case KBActionSheetArrowDirectionUp:
                ...
                break;
            case KBActionSheetArrowDirectionRight:
                CGPathMoveToPoint(arrowPath, NULL, CGRectGetMinX(arrowRect), CGRectGetMinY(arrowRect));
                CGPathAddCurveToPoint(arrowPath, NULL,
                ...
                break;
            case KBActionSheetArrowDirectionLeft:
            default:
...
- (void)setUpRegularButtons
{
    CGRect buttonFrame = (CGRect){0,0,self.width,KBActionSheetButtonHeight};
...
- (void)setUpCancelButton
{
    ...
    self.cancelButton.frame = (CGRect){0,0,self.width,KBActionSheetButtonHeight};
...
- (void)setUpDestructiveButton
{
    ...
    self.destructiveButton.frame = (CGRect){0,0,self.width,KBActionSheetButtonHeight};
...
- (void)setUpHeaderView
{
    ...
    if (self.headerView) {
        ...
        frame.size = [titleLabel sizeThatFits:(CGSize){self.width-(KBActionSheetTitlePadding*2.0f), NSIntegerMax}];
        frame.origin = (CGPoint){KBActionSheetTitlePadding, KBActionSheetTitlePadding};
        
        CGRect backgroundFrame = self.headerBackgroundView.frame;
        backgroundFrame.size.width = self.width;
        backgroundFrame.size.height = frame.size.height + (KBActionSheetTitlePadding * 2.0f);
...
- (void)setUpContainerWidth
{
    if (self.compactLayout == NO) {
        self.width = KBActionSheetDefaultWidth;
        return;
    }
...
- (void)setUpContainer
{
    ...
    frame.size.height += (self.buttonTitles.count * KBActionSheetButtonHeight);
    ...
    if (self.headerBackgroundView)
        ...
        height = KBActionSheetButtonHeight;
...
- (void)addButtonWithTitle:(NSString *)title atIndex:(NSInteger)index tappedBlock:(void (^)(void))tappedBlock
{
    if (title.length == 0 || tappedBlock == nil) {
        [NSException raise:NSInternalInconsistencyException format:@"KBActionSheet: Buttons must have both a block and a title."];
    }
...
- (KBActionSheetArrowDirection)bestArrowDirectionForPresentationRect:(CGRect)presentationRect
{
    ...
    CGFloat height = CGRectGetHeight(self.containerView.frame) + KBActionSheetArrowHeight;
    CGFloat width = KBActionSheetDefaultWidth + KBActionSheetArrowHeight;
    
    //Go in a counter-clockwise direction
    if (CGRectGetMinY(presentationRect) - height > 0)
        return KBActionSheetArrowDirectionDown; //Appears on top
    else if (CGRectGetMinX(presentationRect) - width > 0)
        return KBActionSheetArrowDirectionRight; //Left hand side
    else if (CGRectGetMaxX(presentationRect) + width <= CGRectGetWidth(self.frame))
        return KBActionSheetArrowDirectionLeft;
    else if (CGRectGetMaxY(presentationRect) + width <= CGRectGetHeight(self.frame))
        return KBActionSheetArrowDirectionUp;

    return KBActionSheetArrowDirectionNone;
}
...
- (CGRect)frameOfContainerViewWithArrowDirection:(KBActionSheetArrowDirection)direction presentationRect:(CGRect)presentationRect
{
    ...
    switch (direction) {
        case KBActionSheetArrowDirectionDown:
            frame.origin.y = CGRectGetMinY(presentationRect) - (CGRectGetHeight(frame) + KBActionSheetArrowHeight);
        ...
        case KBActionSheetArrowDirectionRight:
            ...
            frame.origin.x = CGRectGetMinX(presentationRect) - (CGRectGetWidth(frame) + KBActionSheetArrowHeight);
            break;
        case KBActionSheetArrowDirectionUp:
            frame.origin.y = CGRectGetMaxY(presentationRect) + KBActionSheetArrowHeight;
            frame.origin.x = CGRectGetMidX(presentationRect) - (CGRectGetWidth(frame) * 0.5f);
            break;
        case KBActionSheetArrowDirectionLeft:
            frame.origin.y = CGRectGetMidY(presentationRect) - (CGRectGetHeight(frame) * 0.5f);
            frame.origin.x = CGRectGetMaxX(presentationRect) + KBActionSheetArrowHeight;
            break;
        default:
            break;
    }
    ...
    if (frame.origin.x < KBActionSheetScreenPadding)
        frame.origin.x = KBActionSheetScreenPadding;
    
    if (frame.origin.y < KBActionSheetScreenPadding)
        frame.origin.y = KBActionSheetScreenPadding;
    
    if (CGRectGetMaxX(frame) > (boundSize.width - KBActionSheetScreenPadding))
        frame.origin.x = (boundSize.width - KBActionSheetScreenPadding) - CGRectGetWidth(frame);
    
    if (CGRectGetMaxY(frame) > (boundSize.height - KBActionSheetScreenPadding))
        frame.origin.y = (boundSize.height - KBActionSheetScreenPadding) - CGRectGetHeight(frame);
...
- (CGRect)frameOfArrowViewWithDirection:(KBActionSheetArrowDirection)direction presentationRect:(CGRect)presentationRect
{
    ...
    switch (direction) {
        ...
        case KBActionSheetArrowDirectionRight:
            ...
            break;
        case KBActionSheetArrowDirectionUp:
            frame.origin.y = -KBActionSheetArrowHeight;
        ...
        case KBActionSheetArrowDirectionLeft:
            offset = CGRectGetMidY(presentationRect) - CGRectGetMidY(containerFrame);
            frame.origin.y = (CGRectGetMidY(containerBounds) - (CGRectGetHeight(frame) * 0.5f)) + offset;
            frame.origin.x = -KBActionSheetArrowHeight;
            break;
        default:
            break;
    }
    ...
    if (direction == KBActionSheetArrowDirectionDown || direction == KBActionSheetArrowDirectionUp) {
        frame.origin.x = MAX(KBActionSheetBorderRadius, frame.origin.x);
        CGFloat minX = (CGRectGetWidth(containerBounds) - KBActionSheetBorderRadius) - KBActionSheetArrowBase;
        frame.origin.x = MIN(minX, frame.origin.x);
    }
    else if (direction == KBActionSheetArrowDirectionLeft || direction == KBActionSheetArrowDirectionRight) {
        frame.origin.y = MAX(KBActionSheetBorderRadius, frame.origin.y);
        CGFloat minY = (CGRectGetHeight(containerBounds) - KBActionSheetBorderRadius) - KBActionSheetArrowBase;
...
- (void)setStyle:(KBActionSheetStyle)style
{
    if (style == _style)
        return;
